...
class SSHConnection {
    ...
    connect(io) {
        ...
        this.currentConnection.on("ready", function() {
                ...
                stream.on("data", function(data) {
                    if(!this.connected) {
                        ...
                        for(var i = 0; i < content.length; i++)
                            if(content[i].includes(resultHeader))
                                startIndex = i;

                        for(var i = startIndex; i < content.length; i++)
                            if(content[i].includes(resultTableHeader))
                                startIndex = i + 2;
                        ...
                        for(var i = startIndex; i < content.length - 1; i++) {
                            if(!content[i].includes(monitoringCommandShort)) {
                                ...
                                if(data.length == 6) {
                                    ...
                                    io.emit("notify_measurements_updated", measurements.getMeasurements());
                                }
                            }
                        }
                    }
            ...
            io.emit("notify_configdb_updated", config.getConfiguration());
            io.emit("notify_exception", JSON.stringify({
                instance: error,
